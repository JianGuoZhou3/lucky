...
FastUpset <- function(data,
  ...
  ## 加载必要的包
  # need <- c("UpSetR","dplyr");Plus.library(need)
  ...
  for(i in 1:length(data)){
    ...
    if(length(data2[[i]]) < t.i){
      LuckyVerbose(paste0(names(data)[i]," is not a unique vector."))}
  }
  ...
  calculate.combn <- function(data1){
    ...
    getcombn1 <- function(data1.i){
      z <- as.character(data1.i)
      z1 <- names(data1.i)[z %in% "1"]
  ...
  #inter <- nrow(comb$freq) # View(comb$freq)
  inter <- length(unique(unlist(Fastextra(names(comb$category),"-"))))
  nbar <- nrow(comb$freq)
  ...
  test.comb <- function(comb){
    ...
    for(i in 1:length(category)){ #i=2
     ...
     lg1 <- all(lg)
     if(lg1){
       if(verbose) LuckyVerbose(paste0(names(category)[i]," correct!"))
     } else {
       if(verbose) LuckyVerbose(paste0(names(category)[i]," error!"))
     }
    }
  }
  if(verbose) LuckyVerbose("correction testing...")
  test.comb(comb)
  ...
  if(is.null(nsets)){
    ...
    if(nsets > inter){
      if(verbose) LuckyVerbose("The nsets you set is too large and had been corrected.")
      nsets <- inter
    } else {
      nsets <- nsets
    }
