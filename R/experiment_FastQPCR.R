...
FastQPCR <- function(data,
  ...
  data2 <- data
  colnames(data2)[Fastmatch(c(group,marker,value,sample),colnames(data2))] <- c("groups","markers","Ct","samples")
  data2$samples <- factor(data2$samples)
  ...
  # New fashion
  p.con <- ggplot(data = data2,aes(x = markers,y = Ct,color = samples)) +
    geom_boxplot() +
    facet_wrap(. ~ groups) +
    theme_bw() +
    ggtitle("qPCR Quality Control") +
