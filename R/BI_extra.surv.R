...
extra.surv <- function(expr.matrix,
  ...
  ## 基因表达量的二分类函数
  fun1 <- function(expr.matrix1,
                   design1,
                   select,
                   mode){
    gene1.expr <- as.numeric(as.matrix(expr.matrix1[select,]))
    two.tie <- function(vt,mode){
      ...
      if(mode != "maxstat"){
        ...
        ##maxstat运算
        LuckyVerbose("maxstat auto cut-off...")
    ...
    sur.vt2 <- data.frame(gene=sur.vt,
                          row.names = colnames(expr.matrix1))
    colnames(sur.vt2) <- select
    l <- list(sur.vt = sur.vt2,cut.off = cut.off)
    return(l)
  }
