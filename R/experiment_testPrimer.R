...
testPrimer <- function(forward,
                       ...
                       verbose = T){
  ## load package
  nd <- c("XML","rvest","httr","jsonlite","xml2","Biostrings");Plus.library(nd)

  ## human or mouse
  if(species == "human"){
    annot.c <- common.annot
    annot.t <- common.annot.transcripts_GRCh38
    gene.pattern <- "ENSG"
  } else {
    if(species == "mouse"){
      annot.c <- mouse.common.GRCm38.p6
      annot.t <- mouse.transcripts_GRCm38.p6
      gene.pattern <- "ENSMUSG"
    } else {
      stop("Please use one of 'human' or 'mouse'.")
    }
  }
  ...
  for(i in 1:length(gene)){
    g.i <- gene[i]
    test <- length(grep(gene.pattern,g.i)) == 0
    if(test){
      # id need to be converted
      g.i <- convert(g.i,
                     fromtype = "SYMBOL",
                     totype = "ENSEMBL",
                     db = annot.c)
  ...
  if(length(path)==0){
    ...
    if(is.null(path.transcript.fa)|is.null(path.exon.fa)){
      ...
      for(i in 1:length(gene)){ # i=1
        g.i <- gene[i]
        tran.i <- annot.t$transcript_id[annot.t$gene_id %in% g.i]
        tran.i <- unique(tran.i[!is.na(tran.i)])
        exon.i <-  annot.t$exon_id[annot.t$gene_id%in% g.i]
      ...
      for(i in 1:length(gene)){ # i=1
        ...
        write.table(tran.fa,file = paste0("./testPrimer/",g.i,"_",convert(g.i,db=annot.c),"_transcrips.fa"),quote = F,col.names = F,row.names = F)
        ## exons
        exon.i <- exons[[i]]
        exon.fa <- NULL
        ...
        write.table(exon.fa,file = paste0("./testPrimer/",g.i,"_",convert(g.i,db=annot.c),"_exons.fa"),quote = F,col.names = F,row.names = F)
  ...
  for(j in 1:length(forward)){ # j=1
    ...
    if(length(grep("ENSG",g.i)) == 0){
      g.i <- convert(g.i,fromtype = "SYMBOL",totype = "ENSEMBL",db=annot.c)
    } else {
      g.i <- g.i
    }
    ...
    l.i <- testPrimer1(forward=forward.i,
                       ...
                       path.exon.fa = exon.p,
                       verbose = F)
    n.i <- paste(g.i,convert(g.i,db=annot.c),paste0("F:",forward.i),paste0("R:",reverse.i),sep = "_")
